...
function toRow(f: ProductFormModel) {
  return {
    ...
    working_load_limit: f.working_load_limit.trim() || null,
    max_user_weight_kg: f.max_user_weight_kg.trim() || null,
    rope_diameter_min_mm: f.rope_diameter_min_mm,
    rope_diameter_max_mm: f.rope_diameter_max_mm,
    serial_number_location: f.serial_number_location.trim() || null,
...
function buildPreview(rows: Record<string, unknown>[]): ImportPreview {
  ...
  rows.forEach((raw, i) => {
    ...
    const f: ProductFormModel = {
      ...
      working_load_limit: String(raw.working_load_limit ?? '').trim(),
      max_user_weight_kg: String(raw.max_user_weight_kg ?? '').trim(),
      rope_diameter_min_mm: numOrNull(raw.rope_diameter_min_mm),
      rope_diameter_max_mm: numOrNull(raw.rope_diameter_max_mm),
      serial_number_location: String(raw.serial_number_location ?? '').trim(),
