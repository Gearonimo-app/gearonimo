<template>
  <div class="cd">
    <AppHeader :title="(customer?.name as string) || $t('customers.title')" back-to="/customers">
      <button v-if="customer && !editMode" class="cd__icon" :title="$t('common.edit')" @click="startEdit"><GIcon name="edit" class="hdr-glyph" /></button>
    </AppHeader>
...
async function onStartInspection() {
  ...
  if (draftInspection.value) {
    ...
    try {
      ...
      const existingIds = await fetchInspectionArticleIds(draftId)
      const scope = await fetchArticleScope(id, existingIds)
  ...
  startingInspection.value = true
  try {
    const scope = await fetchArticleScope(id)
    if (!scope.allIds.length) {
...
<style scoped>
.cd { min-height: var(--page-min-h, 100vh); background: #f0f4f8; display: flex; flex-direction: column; }
.cd__header {
